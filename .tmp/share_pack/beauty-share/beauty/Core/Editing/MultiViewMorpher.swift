...
struct MultiViewMorpher {
    static func solve(front: UIImage,
        ...
        // 鼻背直化：在鼻梁路径上做轻量 TPS-like 形变，将局部点朝直线投影方向位移
        if let amt = controls["bridge_straighten"], amt > 0.001 {
            func anchorsFor(_ lmk: FacialLandmarksResult?) -> [Anchor] {
                guard let arr = lmk?.points["noseCrest"], arr.count >= 3 else { return [] }
                // 取三点：上/中/下，向直线方向压平
                let a = arr.first!, c = arr[arr.count/2], b = arr.last!
                // 直线方向（a→b）
                let dir = CGPoint(x: b.x - a.x, y: b.y - a.y)
                let len = max(1e-6, hypot(dir.x, dir.y))
                let ux = dir.x/len, uy = dir.y/len
                // 中点到直线的垂距，往直线上投影，位移比例按 amt
                func project(_ p: CGPoint) -> CGPoint {
                    let t = ((p.x - a.x)*ux + (p.y - a.y)*uy)
                    return CGPoint(x: a.x + ux*t, y: a.y + uy*t)
                }
                let da = project(a)
                let db = project(b)
                let dc = project(c)
                return [
                    Anchor(src: CGPoint(x: a.x, y: a.y), dst: CGPoint(x: a.x + (da.x - a.x)*amt, y: a.y + (da.y - a.y)*amt)),
                    Anchor(src: CGPoint(x: c.x, y: c.y), dst: CGPoint(x: c.x + (dc.x - c.x)*amt, y: c.y + (dc.y - c.y)*amt)),
                    Anchor(src: CGPoint(x: b.x, y: b.y), dst: CGPoint(x: b.x + (db.x - b.x)*amt, y: b.y + (db.y - b.y)*amt))
                ]
            }
            if let lmk = frontLandmarks {
                let anchors = anchorsFor(lmk)
                if !anchors.isEmpty { f = TPSDeformer.warp(image: f, anchors: anchors) }
            }
            if let lmk = lmkL {
                let anchors = anchorsFor(lmk)
                if !anchors.isEmpty { l = TPSDeformer.warp(image: l, anchors: anchors) }
            }
            if let lmk = lmkR {
                let anchors = anchorsFor(lmk)
                if !anchors.isEmpty { r = TPSDeformer.warp(image: r, anchors: anchors) }
            }
        }
        return (f, l, r)
    }
