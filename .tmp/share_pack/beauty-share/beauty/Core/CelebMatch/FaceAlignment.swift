...
enum FaceAlignment {
    static func similarityTransform(from src: [CGPoint], to dst: [CGPoint]) -> CGAffineTransform {
        ...
        let X = zip(sx, sy).map { pair -> SIMD2<Double> in SIMD2<Double>(pair.0 - msx, pair.1 - msy) }
        let Y = zip(dx, dy).map { pair -> SIMD2<Double> in SIMD2<Double>(pair.0 - mdx, pair.1 - mdy) }
