...
struct Face3DPreviewView: View {
    ...
    private func makeScene(from mesh: FaceMesh3D) -> SCNScene {
        ...
        let node = SCNNode()
        // Before/After blend
        let after = mesh.deformed(by: ["tip_rotation": paramTip, "bridge_straighten": paramBridge])
        let blended = FaceMesh3D.blend(mesh, after, t: Float(beforeAfter))
        let verts = blended.vertices.map { v -> SCNVector3 in
        ...
        let mat = SCNMaterial()
        if let tex = blended.albedo { mat.diffuse.contents = tex } else { mat.diffuse.contents = UIColor.systemTeal }
