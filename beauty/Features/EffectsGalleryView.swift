...
struct EffectsGalleryView: View {
    ...
    var prefilledControls: [String: Double]? = nil
        ...
        List {
            ...
            Section("效果列表") {
                ForEach(center.activeEffects.filter { categoryFilter == nil ? true : ($0.category == categoryFilter!) }) { pack in
                    NavigationLink(pack.display_name) { EffectDetailView(pack: pack, initialControls: prefilledControls) }
                }
                if center.activeEffects.isEmpty {
...
struct EffectDetailView: View {
    let pack: EffectPack
    var initialControls: [String: Double]? = nil
    ...
    var body: some View {
        ...
        .task {
            if let initc = initialControls, controlValues.isEmpty {
                controlValues = initc
            }
