...
struct ProofProducer {
    ...
    static func runBoth(autoShare: Bool) async {
        ...
        // Doctor mode demo (mesh + overlay + 2 sliders)
        do {
            if let m = CaptureStore.shared.lastMesh {
                let base = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent("proof/doctor", isDirectory: true)
                try? FileManager.default.createDirectory(at: base, withIntermediateDirectories: true)
                let url = base.appendingPathComponent("doctor_mode_demo.mp4")
                try await SceneRecorder.record(mesh: m, duration: 10, output: url)
            }
        } catch { }
        // Edge recon demo
        do {
            let url = try await produceEdgeReconDemo()
            DebugLog.log("edge_recon_demo at: \(url.path)")
        } catch { DebugLog.log("edge_recon_demo_error: \(error.localizedDescription)") }
    ...
    // MARK: - Edge Recon Demo
    static func produceEdgeReconDemo() async throws -> URL {
        // Seed tri-view samples and run remote edge
        let _ = try? await TriViewSampleProvider.reconstructMesh()
        if let m = CaptureStore.shared.lastMesh {
            let base = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent("proof/edge_recon", isDirectory: true)
            try? FileManager.default.createDirectory(at: base, withIntermediateDirectories: true)
            let videoURL = base.appendingPathComponent("demo.mp4")
            try await SceneRecorder.record(mesh: m, duration: 8, output: videoURL)
            // Diagnostics snapshot
            let diag = DiagnosticsSnapshotter.snapshot(mode: .mockTrueDepth)
            try diag.pngData()?.write(to: base.appendingPathComponent("diagnostics.png"))
            // Write last_job.json if exists (+ augment)
            let cache = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0].appendingPathComponent("ReconCache", isDirectory: true)
            if let id = UserDefaults.standard.string(forKey: "edge_job_id") {
                let last = cache.appendingPathComponent("\(id)/last_job.json")
                if FileManager.default.fileExists(atPath: last.path) {
                    let dest = base.appendingPathComponent("last_job.json")
                    var obj: [String: Any] = (try? JSONSerialization.jsonObject(with: Data(contentsOf: last)) as? [String: Any]) ?? [:]
                    obj["provider"] = (UserDefaults.standard.string(forKey: "edge_provider") ?? "-")
                    obj["job_id"] = id
                    obj["latency_ms"] = UserDefaults.standard.double(forKey: "edge_latency_ms")
                    obj["sizes"] = [
                        "glb": UserDefaults.standard.integer(forKey: "edge_glb_size"),
                        "texture": UserDefaults.standard.integer(forKey: "edge_tex_size")
                    ]
                    obj["cache_path"] = cache.appendingPathComponent(id).path
                    if obj["metrics"] == nil { obj["metrics"] = [:] }
                    // sha256
                    let demo = videoURL.path
                    let diag = base.appendingPathComponent("diagnostics.png").path
                    let glb = cache.appendingPathComponent("\(id)/mesh.glb").path
                    let tex = cache.appendingPathComponent("\(id)/texture.png").path
                    obj["sha256"] = [
                        "demo_mp4": Hasher.sha256Hex(filePath: demo) ?? "",
                        "diagnostics_png": Hasher.sha256Hex(filePath: diag) ?? "",
                        "mesh_glb": Hasher.sha256Hex(filePath: glb) ?? "",
                        "texture_png": Hasher.sha256Hex(filePath: tex) ?? ""
                    ]
                    if let data = try? JSONSerialization.data(withJSONObject: obj, options: [.prettyPrinted]) { try? data.write(to: dest) }
                }
            }
            return base
        }
        throw NSError(domain: "EdgeDemo", code: -1, userInfo: [NSLocalizedDescriptionKey: "No mesh available"])
    }
